...
### macOS 断 WiFi 后 TUN DNS 问题 — 已解决(2026-07-14)

**问题**:desktop TUN 模式(utun 虚拟网卡)断 WiFi 后 DNS 解析失败(curl "Could not resolve host"),但 IP 直连 + dig @4G DNS 均成功。

**根因**:macOS 断 WiFi 后 configd 清除 `resolver #1`(全局)的 nameserver。用户态 TUN(wireguard/tun 创建的 utun)不是 configd 管理的 network service,configd 不自动为其配置 DNS。所有 workaround(networksetup / /etc/resolver / scutil Global DNS / 127.0.0.1 proxy)均无效。

**解决方案**:scutil 注入 synthetic SCDynamicStore service(Tailscale + ZeroTier 方法)。

**关键**:仅设 DNS 会被标记 `Supplemental`(非 primary resolver)。必须同时注入 **DNS + IPv4 + IPv6** state,configd 才把 synthetic service 当 primary network service。

**tun-helper 实现**(`desktop/cmd/tun-helper/main.go`):
```go
const syntheticServiceKey = "State:/Network/Service/FF457792-79C0-4A25-8392-D875BBEACCA6"

// 1. DNS:SearchOrder 100000(高优先级)+ SupplementalMatchDomains ""(全域)
// 2. IPv4:4G IP + InterfaceName(utun)+ Router 127.0.0.1
// 3. IPv6:fe80::dji4g link-local(让 macOS 认为有 IPv6 连通性)
```

**验证结果**:断 WiFi + TUN 模式,curl baidu 成功(系统 DNS resolver 正确路由到 4G DNS)。用户实测通过 4G SIM 卡发送消息。
...
- 本地 DNS proxy 127.0.0.1:53(proxy 正常但 resolver 不路由)
- 仅 DNS synthetic service(被标记 Supplemental,非 primary)

**参考**:
- [Tailscale manager_darwin.go](https://github.com/tailscale/tailscale/blob/main/net/dns/manager_darwin.go) — `setGlobalDNS` + `SearchOrder 100000`
- [ZeroTier MacDNSHelper.mm](https://github.com/zerotier/ZeroTierOne/blob/main/osdep/MacDNSHelper.mm) — `addIps4`/`addIps6`(IPv4/IPv6 state)

**备选方案**(调研已完成,留作参考):
- NetworkExtension(Tailscale GUI/WireGuard 官方)→ 需 $99/年 + Swift bridge(docs/14)
- feth 虚拟以太网(ZeroTier)→ Layer 2,需改数据面(docs/15)
