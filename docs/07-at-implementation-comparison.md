...
> **已合并到 `docs/10-at-commands-alignment.md`**——该文档同时包含本节的大类评分(A-E)、
> 逐命令清单(133 条)和上游溯源(SG/VH 对照),是统一的 AT 对齐追踪文档。
>
> 本文 §一–§六 保留为**历史选型对比快照**(2026-07-11,纯 SG vs vohive),不再更新。
> 查当前对齐状态请看 `docs/10` §三(逐命令)+ §三·二(SG/VH 溯源)+ §七(大类评分)。
