...
> 隶属 `plans/03/00-overview.md`。依赖子计划 02(Bridge)+ 03(集成)。
> 创建于 2026-07-12。
...
type fakeBulkWriter struct{ mu sync.Mutex; written [][]byte }
...
| `TestRelayOffsetMacOS` | offset=4 时 macOS headroom 正确处理 |
| `TestBridgeStopWaitsGoroutines` | Stop 等 relay goroutine 退出(不泄漏) |
...
func TestHardwareRelayEndToEnd(t *testing.T) {
    // 1. 拨号(manager.NewWithClient + Connect)
    // 2. 创建 TUN
...
- `internal/qmidatapath/relay.go` —— 子计划 02,被测代码
- `internal/testutil/scriptport.go` —— 现有 mock(参考模式,但 relay 用专门的 fakeBulkReader/Writer)
- `internal/usbtransport/usbtransport_test.go` —— mock 测试模式参考(scriptReader 适配器)
