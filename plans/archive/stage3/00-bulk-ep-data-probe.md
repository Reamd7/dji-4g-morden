...
- [x] WDA SetDataFormat 在 QDC507 上成功 — modem 收发 raw IP,无 QMAP 头
- [x] WDS StartNetwork 成功(IP + PDH,与阶段 2 一致)
- [x] **bulk IN EP 0x88 读到 raw IP 数据(version=4 和 6)** — 核心验证通过 ✅
- [x] **手动发 IP 包到 bulk OUT 0x05,从 bulk IN 0x88 收到 reply** — TX+RX 双向确认 ✅
- [x] **ZLP 测试:512 字节包需 ZLP** — 28B 包有 reply,512B 包无 reply,zlp=true 确认 ✅

### 实测结论(3 次运行,最终用 114DNS)

**Run 1**(8.8.8.8,被墙):Phase C 收到 4 个 IPv6 RA 包(0x60...,104B,ICMPv6 link-local)。
TX 测试因 8.8.8.8 被墙无 reply,无法区分 ZLP vs 运营商过滤。

**Run 2**(8.8.8.8,被墙):Phase C 0 包(错过 RA 周期)。Phase D/D2 均无 reply(同上)。

**Run 3**(114.114.114.114,最终):Phase C 0 包。Phase D 28B ICMP echo → **收到 512B IPv4 reply**
(`45 00 02 00...6b 01...src=8.8.8.8 dst=10.147.x.x`)。Phase D2 512B 包无 reply。

**R1 PASS**:bulk IN EP 0x88 承载 raw IP(IPv6 RA + IPv4 ICMP reply,无 QMAP 头)。

**R2 PASS**:WDA SetDataFormat 成功(modem 收发 raw IP = WDA 已协商)。

**TX 通路 PASS**:28B ICMP echo via bulk OUT 0x05 → 512B IPv4 reply via bulk IN 0x88。
TX 和 RX 双向工作。

**R5 ZLP 确认**:28B 包(非 512 对齐)有 reply,512B 包(512 对齐)无 reply。
差别恰好是 `len % maxPacketSize == 0`。**modem 需要 ZLP**。
`zlp=true` 确认(匹配 Linux 驱动 FLAG_SEND_ZLP)。

**阶段 3 门控全部通过**:raw-IP relay 方案成立,子计划 01 解除阻塞。
