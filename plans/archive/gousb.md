...
- [x] MI_02(AT 口)的 bulk IN / bulk OUT endpoint 地址 → EP 0x03 OUT / 0x84 IN(AGENTS.md 实测确认)
- [x] MI_04(QMI)的 bulk IN / bulk OUT / interrupt IN endpoint 地址 → EP 0x05 OUT / 0x88 IN / 0x89 intr(AGENTS.md 实测确认)
- [x] Zadig 装 WinUSB 后,gousb 能否成功 claim MI_02 → ✅ 成功(AT 通路已打通)
- [x] claim MI_02 后,`AT\r\n` → `OK` 是否返回(USB→AT 通路验证) → ✅ 返回 OK(cmd/attest + 硬件测试验证)

### 5.2 留到后续步骤再定

- [x] USB transport 的 deadline 实现细节 → 已解决:方向 F 用长生命期阻塞读(Close 时才 cancel),不需要 deadline 链
- [x] `internal/at/at.go` 的具体分帧/URC 分流逻辑 → 已解决:直接用 SG modem 的 readerLoop,不自写 AT 分帧
- [x] 静态 vs 动态链接 libusb 的最终决策 → 动态链接(gousb cgo + mingw libusb-1.0.dll)
