...
> 隶属 `plans/03/00-overview.md`。**阶段 3 核心**(数据中继层)。
> 依赖子计划 01 通过(raw-IP 确认 + WDA 确认)。
> 创建于 2026-07-12。
...
新建 `internal/qmidatapath/` 包,实现 TUN 虚拟网卡与 QMI bulk endpoint 之间的双向 raw IP 中继。这是整个阶段 3 的核心交付物——上游 quectel-qmi-go 没有这层(它依赖内核 qmi_wwan),我们纯用户态必须自己写。

---

## 二、依赖

- **子计划 01 通过**:raw-IP 确认(首字节 IP version=4/6)。如果探针发现是 QMAP,本计划 §六 加 strip/add 层。
- 引入 `golang.zx2c4.com/wireguard/tun`(`go get`,隐式带 `golang.zx2c4.com/wintun` Windows 依赖)
...
- 模块路径:`golang.zx2c4.com/wireguard`,子包 `tun`
- 版本:日期版本号(最新约 `0.0.20250522`,无 2026 版)
- 三平台:Linux `/dev/net/tun`、macOS utun、Windows Wintun.dll
- layer-3(IP only,无以太文头)——与 modem raw-IP 完全匹配
- **Windows 额外**:`golang.zx2c4.com/wintun` 隐式依赖,运行时需 wintun.dll 放 exe 同目录(子计划 03)
...
- **offset**:macOS utun 需 4 字节 headroom(protocol family 头),用 offset=4 或 `tun.Offset` 常量;Linux/Windows offset=0
- **Windows BatchSize()=1**:每次只读/写 1 个包,relay 循环按单包处理(跨平台一致)
- **macOS 命名不受控**:传 "qmi0" 被改成 "utunN",relay 配置要用 `Device.Name()` 实际返回值
...
type Bridge struct {
    ...
    mtu     int           // 1500
    zlp     bool          // 子计划 01 D2 结果:是否需 ZLP

    cancel  context.CancelFunc
    done    chan struct{}  // 两个 relay goroutine 都退出后关闭
    mu      sync.Mutex
    started bool
}

// New creates a Bridge. tun/bulkIn/bulkOut must be pre-opened. offset from
// runtime.GOOS (macOS=4). zlp from subplan 01 D2 probe result.
...
// Stop cancels the relay context and waits for both goroutines to exit, then
// closes the TUN. Safe to call after Start.
...
func (b *Bridge) tunToModem(ctx context.Context) {
    ...
    for {
        ...
        if err != nil { /* log, continue or return on fatal */ continue }
        for i := 0; i < n; i++ {
            pkt := bufs[i][b.offset : b.offset+sizes[i]]
            if _, err := b.bulkOut.Write(pkt); err != nil { /* log */ continue }
...
func (b *Bridge) modemToTun(ctx context.Context) {
    ...
    for {
        ...
        if err != nil { /* timeout/context = continue; fatal = log */ continue }
        if n == 0 { continue }
...
### 5.4 raw-IP 直传(子计划 01 确认后的默认路径)

子计划 01 探针确认 raw-IP 后,bulk EP 上 = 裸 IP 包,TUN 也是 layer-3。**relay 直接转发,无需任何头处理**。这是最简实现。
...
- **Close 时序**:`Bridge.Stop()` 先 cancel relay context(等两个 goroutine 退出)→ 再 `QMITransport.Close()`(释放 iface)。绝不能反过来(释放 iface 时 relay 还在读写 → segfault)

---

## 六、QMAP 降级(如果子计划 01 发现非 raw-IP)
...
- [ ] mock 单测(`relay_test.go`):双向转发、ZLP、offset、Close 时序(子计划 04)
- [ ] Bridge.Start/Stop 生命周期正确(Stop 等 goroutine 退出)
- [ ] raw-IP 直传路径工作(子计划 01 确认格式后)
...
| TX ZLP(R5):512 倍数包卡 | `b.zlp` 标志从子计划 01 D2 取。gousb 0 字节 Write 行为需实测 |
...
- `internal/qmitransport/bulkendpoints.go` —— 子计划 01,OpenBulkEndpoints 返回的 *gousb.InEndpoint/*OutEndpoint 天然满足 BulkReader/Writer
