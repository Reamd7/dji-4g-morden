...
- [x] `cmd/qmidial/main.go` 加 `-tun` 标志(commit f9b7625)
- [x] TUN 创建 + Bridge 启动 + IP/路由自动配(manager.configureNetwork)
- [x] DNS 三平台自建(Windows netsh / macOS networksetup / Linux resolvectl)
- [x] Windows wintun.dll + libusb-1.0.dll 分发(exe 同目录,admin UAC 无 mise PATH)
- [x] **curl http://www.baidu.com → HTTP 200 in 141ms(TCP 走 4G 上网成功)**
- [x] DNS 解析:`nslookup baidu.com` → 解析成功(110.242.74.102 等)
- [ ] ping 114.114.114.114 → 100% loss(**运营商(运营商)数据 APN 过滤 ICMP**,非 relay 问题)

### 实测结论

**relay 双向工作确认**:
- TCP: curl baidu.com → HTTP 200, 141ms(上行请求 + 下行响应都通过 relay)
- UDP: nslookup baidu.com → DNS 解析成功(查询 + 响应都通过 relay)
- ICMP: ping 失败 — 运营商 CGNAT 数据 APN 过滤 ICMP echo,与 relay 无关

**wintun.dll 加载修复**:
- wintun-go 用 `LOAD_LIBRARY_SEARCH_APPLICATION_DIR` 找 DLL,在 gousb cgo 环境下解析失败
- 修复:`wintun_preload_windows.go` 用全路径 `LoadLibraryEx` 预加载
- admin UAC 提升的进程没有 mise 的 PATH,需 `libusb-1.0.dll` 也放 exe 同目录
